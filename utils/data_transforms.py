...
class Resize(object):
    ...
    def __call__(self, rendering_images):
        # processed_images = np.empty(shape=(0, self.img_size[0], self.img_size[1], 3))
        processed_images = np.empty(shape=(0, self.img_size[0], self.img_size[1], rendering_images.shape[-1]))
...
class ToTensorV2(object):
    ...
    def __call__(self, rendering_images):
        assert (isinstance(rendering_images, np.ndarray))
        array = np.transpose(rendering_images, (2, 0, 1))
        # handle numpy array
        tensor = torch.from_numpy(array)
