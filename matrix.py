...
from data import ProteinDataset, SequenceDataset, ProteinAutoEncoderDataset, ProteinTransformDataset
from VL_trainer import Model, OnlySeqModel, AutoEncoder, PretrainedAE_Model, AutoEncoder_old, Matrix_Model
...
def main(cfg):
	...
	if not cfg.DATASET.AUTOENCODER and not cfg.TRAIN.ONLYSEQ:
		train_dataset = ProteinTransformDataset(cfg, 'train', cfg.CONST.N_VIEWS_RENDERING, transforms = train_transforms, grayscale=cfg.DATASET.GRAYSCALE, big_dataset=cfg.DATASET.BIGDATA)
		val_dataset = ProteinTransformDataset(cfg, 'val', cfg.CONST.N_VIEWS_RENDERING, transforms = val_transforms, grayscale=cfg.DATASET.GRAYSCALE, big_dataset=cfg.DATASET.BIGDATA)
